...
def check_users_assigments(
    ...
    users_authorizations: Dict[Tuple[str, str, str, Optional[str]], List[str]] = (
        get_authorizations_unique(
    ...
    if not any(users_authorizations.values()):  # no user's roles in DB
        ...
        if assignments_only_mode:
            return {
                ('', user, '', ''): "WARNING: User haven't got any roles" for user in users
            }, None
        return_message: str = "Users haven't got any roles in 1C Srvr=%s, Ref=%s." % (
            host,
            database,
        )
        message: str = '%s List:\n%s' % (
    ...
    if not retrospective_mode:
        users_differense: Set[Optional[str]] = set(users) - set(
            user for _, user, *_ in users_authorizations
        )
